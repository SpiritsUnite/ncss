# NOTE: This code does NOT work. Use Q6's instead!
...
def bfsolve(at, predicates, subs):
                    ...
                    for pred in nPreds:
                        for k, v in enumerate(pred[1]):
                            if arg(v):
                                ...
                                if variable(pred[1][k]) and v in subs:
                                    pred[1][k] = subs[v]
    ...
    poss = bfsolve(at + 1, predicates, poss)
    return poss
...
for line in open("program.plg"):
    ...
    if line.startswith("?-"):
        ...
        ans = bfsolve(0, [predicate], [])
        if ans:
            printed = False
            for i in sorted(predicate[1]):
                if variable(i):
                    print "{0} = {1}".format(i, ans[0][i])
                    printed = True
            if not printed:
                print "yes"
        else:
